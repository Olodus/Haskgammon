...
setup (Interface hm _ ) initBoard =
  startGUI defaultConfig (baseHTML initBoard (hm))
...
baseHTML initBoard gameLogic window  = do
     ...
     hidden1 <- mkInvis "i1" "" --Used for knowing value of first click
     ...
     -- Layout,
     pointsFirstRow <-
       row ([ pure b0, pure b1, pure b2,
              pure b3, pure b4, pure b5,
              pure b6, pure b7, pure b8,
              pure b9, pure b10, pure b11])

     pointsSecondRow <-
       row ([pure b12, pure b13, pure b14,
             pure b15, pure b16, pure b17,
             pure b18, pure b19, pure b20,
             pure b21, pure b22, pure b23])

     hiddens <- row ([pure hidden1])
     getBody window #+
       [column [pure pointsFirstRow,
                pure pointsSecondRow,
                pure hiddens]]
...
createCanvasBoard =
 return ([
   (mkButton (Just White) "2" 0),(mkButton Nothing "" 1),
   (mkButton Nothing "" 2),(mkButton Nothing "" 3),
   (mkButton Nothing "" 4),(mkButton (Just Black) "5" 5),
   (mkButton Nothing "" 6),(mkButton (Just Black) "3" 7),
   (mkButton Nothing "" 8),(mkButton Nothing "" 9 ),
   (mkButton Nothing "" 10),(mkButton (Just White) "5" 11),
   (mkButton (Just Black) "5" 12),(mkButton Nothing "" 13),
   (mkButton Nothing "" 14),(mkButton Nothing "" 15),
   (mkButton (Just White) "3" 16),(mkButton Nothing "" 17),
   (mkButton (Just White) "5" 18),(mkButton Nothing "" 19),
   (mkButton Nothing "" 20),(mkButton Nothing "" 21),
   (mkButton Nothing "" 22),(mkButton (Just Black) "2" 23)
 ])
...
parseState window =
  (GameState False
    (Player White Human) (Player Black Human) (Board [Point([]),
...
handleInput :: (Move -> GameState -> StdGen -> GameState) -> Window -> String -> UI Element
handleInput gl window id = do
     getBody window # set style [("backgroundColor","lightgray"),
                                ("textAlign","center")]
